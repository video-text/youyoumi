...
if (contactForm instanceof HTMLFormElement) {
  contactForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!response.ok || !result.ok) throw new Error(result.error || "Unable to send your inquiry.");
      contactForm.reset();
      if (status) status.textContent = "Thank you. Your inquiry has been received.";
    } catch (error) {
      if (status) status.textContent = error instanceof Error ? error.message : "Unable to send your inquiry.";
    } finally {
